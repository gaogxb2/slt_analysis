"""Scan testdata directory and seed database."""
import sys
from pathlib import Path

sys.path.insert(0, str(Path(__file__).resolve().parent))

from app.config import TESTDATA_DIR, TESTLOG_DIR
...
def main():
    ...
    db = SessionLocal()
    try:
        result = scan_directory(db, TESTDATA_DIR)
        print(f"SUM OK: {len(result['ok'])} files")
        if result["errors"]:
            print(f"SUM Errors: {result['errors']}")
        log_result = scan_log_directory(db, TESTLOG_DIR)
